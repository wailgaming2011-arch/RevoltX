...
for f in os.listdir("."):
    if f.endswith(".html"):
        shutil.move(f, os.path.join("templates", f))
        print(f"✓ Déplacé: {f}")

# ─────────────────────────────────────────
# auth.html
# ─────────────────────────────────────────
auth = '''{% extends "base.html" %}
...
.auth-bg{position:absolute;inset:0;background:radial-gradient(ellipse 60% 60% at 30% 50%,rgba(230,57,70,0.08),transparent 60%),radial-gradient(ellipse 40% 40% at 70% 50%,rgba(230,57,70,0.05),transparent 60%);}
...
.pf-role{color:var(--text2);font-size:.9rem;margin:.2rem 0;}
...
# competitions.html
# ─────────────────────────────────────────
competitions = '''{% extends "base.html" %}
...
.comp-meta{display:flex;gap:1.5rem;font-size:.85rem;color:var(--text2);flex-wrap:wrap;}
    ...
    <div class="product-card">
      ...
      <div class="product-body">
        ...
        <div class="product-footer">
          ...
          <button class="btn-buy" onclick="addToCart(this.closest('.product-card').querySelector('.product-name').innerText)">Acheter</button>
        </div>
      </div>
    </div>
    {% endfor %}
...
function addToCart(name){
  ...
  msg.style.cssText='position:fixed;top:80px;right:1.5rem;z-index:9999;padding:.75rem 1.2rem;background:#06d6a020;border:1px solid #06d6a060;color:#06d6a0;border-radius:6px;font-weight:600;font-family:Rajdhani,sans-serif;';
  msg.innerHTML='&#10003; '+name+' ajouté au panier !';
  document.body.appendChild(msg);
  setTimeout(function(){msg.style.opacity='0';msg.style.transition='opacity .4s';setTimeout(function(){msg.remove();},400);},2500);
}
</script>
{% endblock %}'''
...
.n-emoji{font-size:3rem;flex-shrink:0;}
.n-cat{font-size:.7rem;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--red);margin-bottom:.4rem;}
...
.n-meta{display:flex;gap:1.5rem;font-size:.8rem;color:var(--text2);margin-top:1rem;}
...
<div class="apply-page">
  ...
  {% else %}
  <div class="epic-note"><span>💡</span><span>Connecte ton <a href="/profile" style="color:#60a5fa">compte Epic Games</a> pour renforcer ta candidature !</span></div>
  {% endif %}
  <div class="apply-card">
    <form method="post" action="/apply">
      <div class="form-row">
        <div class="form-group"><label>Jeu</label><select name="game" required><option value="">Choisir...</option><option value="Fortnite">Fortnite</option><option value="Valorant">Valorant</option><option value="CS2">CS2</option><option value="Rocket League">Rocket League</option></select></div>
        <div class="form-group"><label>Rôle</label><select name="role" required><option value="">Choisir...</option><option value="IGL">IGL</option><option value="Fragger">Entry Fragger</option><option value="Support">Support</option><option value="Sniper">AWPer / Sniper</option><option value="Flex">Flex</option></select></div>
      </div>
...
.p-email{color:var(--text2);font-size:.9rem;margin:.2rem 0 .5rem;}
...
.connect-form input::placeholder{color:var(--text2);}
...
<div class="profile-page">
  ...
  <div class="epic-section">
    ...
    <form class="connect-form" method="post" action="/connect-epic" style="margin-top:1rem">
      <input type="text" name="epic_username" placeholder="Changer de compte..." value="{{ user.epic_username }}">
...
.btn-accept{background:rgba(6,214,160,.15);border:1px solid rgba(6,214,160,.4);color:#06d6a0;padding:.4rem .9rem;border-radius:5px;font-family:'Rajdhani';font-weight:700;font-size:.8rem;text-decoration:none;transition:all .2s;}
.btn-reject{background:rgba(230,57,70,.1);border:1px solid rgba(230,57,70,.3);color:var(--red);padding:.4rem .9rem;border-radius:5px;font-family:'Rajdhani';font-weight:700;font-size:.8rem;text-decoration:none;transition:all .2s;}
      ...
      <div class="app-row">
        <div class="app-row-header" onclick="toggleApp('app-{{ app.id }}')">
          <div class="app-row-main">
            ...
            <p>{{ app.date }}{% if app.epic_username %} · 🎮 <b style="color:#60a5fa">{{ app.epic_username }}</b>{% endif %}</p>
          </div>
        ...
        <div class="app-detail" id="app-{{ app.id }}">
          ...
          {% else %}<p style="color:var(--text2);font-size:.85rem">Aucune stat Epic Games liée.</p>{% endif %}
        </div>
      </div>
      {% endfor %}
    {% else %}
...
files = {
    "templates/auth.html": auth,
    "templates/team.html": team,
    "templates/competitions.html": competitions,
    "templates/shop.html": shop,
    "templates/news.html": news,
    "templates/apply.html": apply_html,
...
print("\n===========================================")
print("  ⚡ RevoltX — Tous les templates créés !")
print("===========================================")
